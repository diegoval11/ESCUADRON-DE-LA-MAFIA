...
function showQuestionLoadError(area, message) {
  area.innerHTML = `
...
async function startQuiz() {
  ...
  function strikeOption(el, index) {
    ...
    if (!currentStrikes.includes(index)) {
      currentStrikes.push(index);
    }
  }
  ...
  if (!quizQuestions?.length) {
    ...
    try {
      ...
      showQuestionLoadError(area, err.message);
      document.getElementById('retryLoadBtn').onclick = () => startQuiz();
      return;
    }
    if (!quizQuestions.length) {
  ...
  function applyFiftyFifty(item) {
    ...
    shuffled.slice(0, toStrike).forEach(el => {
      strikeOption(el, parseInt(el.dataset.i, 10));
    });
  ...
  function updateLifelineButtons(item) {
    ...
    const can5050 = !lifelines.fiftyFifty && !answered && activeWrong > 1;
    const canElim = !lifelines.eliminate && !answered && activeWrong > 0 && selectable > 2;

    b5050.disabled = !can5050;
    bElim.disabled = !canElim;
  ...
  function render() {
    ...
    document.getElementById('quizPauseBtn').onclick = () => {
      persistProgress();
      backToMenu();
    };
...
async function startTimed() {
  ...
  try {
    ...
    showQuestionLoadError(area, err.message);
    document.getElementById('retryLoadBtn').onclick = () => startTimed();
    return;
  }
